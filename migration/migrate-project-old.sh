...
case "$ACTION" in
        1)
                ...
                SRC_SUPABASE_URL="https://${SRC_REF}.supabase.co" SUPABASE_URL_OVERRIDE="https://${SRC_REF}.supabase.co" node "$SCRIPT_DIR/export-storage.js" || {
                    echo "⚠️ Storage export failed - continuing with database export"
                }
    ...
    2)
        ...
        psql "$NEW_DB_CONN" -f "$IMPORT_DIR/schema_export.sql" >/dev/null 2>&1 || {
            echo "⚠️ Some schema import warnings (normal for constraints/indexes)"
        }
        ...
        echo "🔍 Run './migration/validate-migration.sh' to verify migration success"
        ;;
    3)
        bash "$SCRIPT_DIR/update-env.sh"
        echo "🔍 After env update, run './migration/validate-migration.sh' to verify"
        ;;
    4)
        echo "Bye"; exit 0; ;;
...
echo ""; echo "🛠️ Additional Tools Available:";
echo "  • ./migration/validate-migration.sh - Verify migration success"
echo "  • ./migration/rollback.sh - Rollback utilities if needed"

echo ""; echo "Next steps:";
case "$ACTION" in
    1)
        echo "  • Create new Supabase project in target account"
        echo "  • Run this script again and choose option 2 (Import)"
        echo "  • Run './migration/validate-migration.sh' to verify migration"
        ;;
    2)
        echo "  • Run option 3 to update environment variables"
        echo "  • Run './migration/validate-migration.sh' to verify migration"
        echo "  • Rebuild and redeploy your application"
        echo "  • Test all features before deleting old project"
        ;;
    3)
        echo "  • Rebuild your application: npm run build"
